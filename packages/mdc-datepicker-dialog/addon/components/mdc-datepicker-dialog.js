import Dialog from "@ivanvanderbyl/ember-material-components-dialog/components/mdc-dialog";
...
export default Dialog.extend({
  ...
  _lookupDateElement(date) {
    const selector = `.fc-content-skeleton td[data-date="${moment(date).format(
      "YYYY-MM-DD"
    )}"]`;
  ...
  actions: {
    ...
    dayRender(date, dayElement) {
      ...
      if (
        (isPresent(min) && srcDate.isBefore(min)) ||
        (isPresent(max) && srcDate.isAfter(max))
      ) {
    ...
    dayClick(date, event) {
      ...
      const dayElement = event.target.classList.contains("fc-day-top")
        ? event.target
        : event.target.parentElement;

      if (
        dayElement.classList.contains(CLASS_NAME_OTHER_MONTH) ||
